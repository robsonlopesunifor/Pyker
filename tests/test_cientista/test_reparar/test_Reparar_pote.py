...
import cientista.Reparar.Reparar_pote as Reparar
from Cenografo.tests.test_cientista.Auxiliar import Auxiliar


class Reparar_pote_test(unittest.TestCase,Auxiliar):

    def test_repara(self):
        print('------------ validar -----------------')
        reparar_pote = Reparar.Reparar_pote()
        dataframe = pd.DataFrame()
        reparar_pote.iniciar(dataframe ,['A' ,'B' ,'C' ,'D' ,'E' ,'F'])
        self.reparar_dicionario_com_telas_embaralhadas(dataframe,reparar_pote,6,3)
        print(reparar_pote.dataframe)
    ...
    def gerador_de_ficheiros(self ,novo_ficheiro):
        if novo_ficheiro == 0:
            ficheiro = {'pote' :0.0 ,'pote_rodada' :1.00 ,'valido_pote' :True ,'mao' :1 ,'bord_etapa' :1,
                        'aposta_A' :1 ,'aposta_B' :1 ,'aposta_C' :1 ,'aposta_D' :1 ,'aposta_E' :1 ,'aposta_F' :1}
        elif novo_ficheiro == 1:
            ficheiro = {'pote' :0.0 ,'pote_rodada' :2.23 ,'valido_pote' :True ,'mao' :1 ,'bord_etapa' :2,
                        'aposta_A' :1.20 ,'aposta_B' :1 ,'aposta_C' :1 ,'aposta_D' :1 ,'aposta_E' :1 ,'aposta_F' :1}
        elif novo_ficheiro == 2:
            ficheiro = {'pote' :0.0 ,'pote_rodada' :5.00 ,'valido_pote' :False ,'mao' :1 ,'bord_etapa' :2,
                        'aposta_A' :1.20 ,'aposta_B' :1.20 ,'aposta_C' :1.20 ,'aposta_D' :1 ,'aposta_E' :1
                        ,'aposta_F' :1}
        elif novo_ficheiro == 3:
            ficheiro = {'pote' :0.0 ,'pote_rodada' :0.0 ,'valido_pote' :False ,'mao' :1 ,'bord_etapa' :3,
                        'aposta_A' :1 ,'aposta_B' :1 ,'aposta_C' :1 ,'aposta_D' :1 ,'aposta_E' :1 ,'aposta_F' :1}
            ...
            ficheiro = {'pote' :0.0 ,'pote_rodada' :0.0 ,'valido_pote' :False ,'mao' :1 ,'bord_etapa' :4,
                        'aposta_A' :1 ,'aposta_B' :1 ,'aposta_C' :1 ,'aposta_D' :1 ,'aposta_E' :1 ,'aposta_F' :1}
        return ficheiro
